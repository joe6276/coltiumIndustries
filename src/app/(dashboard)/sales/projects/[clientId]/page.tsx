// src/app/(dashboard)/sales/projects/[clientId]/page.tsx
'use client'
...
import { ClientPaymentHistory, SalesProjectWithClient } from '@/lib/types/api'
...
export default function SalesClientPaymentDetailsPage() {
  ...
  const fetchClientInfo = async () => {
    try {
      // Fetch sales projects using the updated structure
      const allProjects = await apiClient.getSalesProjects()
      
      // Find the client data using the new clientId field
      const clientData = allProjects.find(client => client.clientId === clientId)
      
      if (clientData) {
        // Calculate totals based on all projects for this client
        const totalValue = clientData.project.reduce((sum, project) => {
          return sum + project.pricing
        }, 0)

        const totalPaid = clientData.project.reduce((sum, project) => {
        ...
        setClientInfo({
          ...
          totalProjects: clientData.project.length,
          totalValue,
          totalPaid,
